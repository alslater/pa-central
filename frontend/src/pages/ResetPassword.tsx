...
export default function ResetPassword() {
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      // A successful reset bumps token_epoch server-side (set_password),
      // revoking every bearer token already issued to this account —
      // including whatever this browser currently holds in localStorage,
      // if the person completing the reset happens to already be signed
      // in as that same account (this route is deliberately reachable
      // while signed in — see its own comment on the missing-token
      // notice). Without clearing the local session here too, "Go to
      // sign in" below navigates to /login while AuthProvider.user is
      // still the stale pre-reset user — Login's own redirect-when-
      // authenticated effect then bounces straight back into the app on
      // a session the backend has already revoked, landing the user in a
      // confusing intermediate state that only resolves once some later
      // request happens to 401. Logging out here makes the advertised
      // next step (sign in with the new password) work deterministically
      // regardless of whether the browser had a session at all.
      logout()
      setDone(true)
    } catch (err: any) {
