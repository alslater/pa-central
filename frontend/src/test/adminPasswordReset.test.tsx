...
// Defaults to the common case: the op is still genuinely pending (no SSE
// event had already arrived) — registerPendingOp's real contract returns
// false only when it resolved an early-arrived event.
vi.mock('@/hooks/useLiveAlerts', () => ({
  useLiveAlertsContext: () => ({
    count: 0, clear: vi.fn(), Toast: null, registerPendingOp: vi.fn(() => true),
    discardEarlyResult: vi.fn(), getSessionEpoch: () => 0,
  }),
}))
...
describe('Users page — admin password reset', () => {
  ...
  it('reports that the password was invalidated and a link is being sent', async () => {
    // reset_link_sent is never true on the wire (dispatch is always
    // backgrounded now) — a genuinely admitted send returns op_id set and
    // reset_link_sent null, reconciled later via the admin-alerts SSE
    // stream; the toast at response time reports the send is in flight,
    // not that it already succeeded.
    const user = setup()
    vi.mocked(api.users.resetPassword).mockResolvedValue({
      password: null, reset_link_sent: null, op_id: 'op-link-sent',
    })
    await openResetPrompt(user)
    await user.click(await screen.findByRole('button', { name: /^reset password$/i }))
    ...
    expect(
      await screen.findByText(/password invalidated — sending reset link/i)
    ).toBeInTheDocument()
  })
  ...
  it('shows the generated password once when no link was sent', async () => {
    const user = setup()
    vi.mocked(api.users.resetPassword).mockResolvedValue({
      password: 'generated-secret-value', reset_link_sent: false, op_id: null,
    })
    await openResetPrompt(user)
    await user.click(await screen.findByRole('button', { name: /^reset password$/i }))
  ...
  it('clears a previous password when a later reset dispatches a link instead', async () => {
    const user = setup()
    vi.mocked(api.users.resetPassword)
      .mockResolvedValueOnce({ password: 'first-generated-password', reset_link_sent: false, op_id: null })
      .mockResolvedValueOnce({ password: null, reset_link_sent: null, op_id: 'op-link-sent' })
    await openResetPrompt(user)
    await user.click(await screen.findByRole('button', { name: /^reset password$/i }))
  ...
  it('shows only the newest password across consecutive resets', async () => {
    ...
    vi.mocked(api.users.resetPassword)
      .mockResolvedValueOnce({ password: 'first-generated-password', reset_link_sent: false, op_id: null })
      .mockResolvedValueOnce({ password: 'second-generated-password', reset_link_sent: false, op_id: null })
    await openResetPrompt(user)
    await user.click(await screen.findByRole('button', { name: /^reset password$/i }))
    expect(await screen.findByText('first-generated-password')).toBeInTheDocument()
