...
async def me(user: User = Depends(get_current_user)) -> UserOut:
    # has_outstanding_welcome_token is always False here, correctly, by
    # construction — not UserOut's schema default going unpopulated (a real
    # bug that was briefly fixed here with users.py's _user_out(), then
    # reverted once this was worked out): a welcome token's only purpose is
    # setting the account's first real password, and the account's password
    # until then is an unknown, never-disclosed random value (register(),
    # self-service branch — body.password is rejected outright when
    # self-service is on). Reaching this endpoint at all means a valid
    # bearer token was issued, which means login_json/login_form already
    # verified the caller knows the CURRENT password hash — which is only
    # possible after the welcome link was used (retiring the token) or an
    # admin separately reset the password (set_password retires any
    # outstanding welcome token too, as part of "every path that changes a
    # password must go through this"). No code path mints a token for a
    # user without them already proving that. So "authenticated" and "holds
    # an unused welcome token for this same account" are mutually exclusive
    # states — computing the real value here would always return False
    # anyway, just at the cost of a query. GET /users (list_users/get_user,
    # via _user_out) still needs that query for a genuine reason: an ADMIN
    # looking at someone ELSE's row, who is not bound by this same
    # invariant. See _user_out's own docstring for the same reasoning from
    # the other side.
    return user
...
@router.post(
    ...
    responses={
        202: {
            "model": RegisterOut,
            "description": (
                "The account was created; the welcome-link email send is "
                "still in flight and its outcome is reported later via SSE."
            ),
        },
    },
)
async def register(
    ...
    The welcome send is backgrounded via dispatch_reset_email, exactly like
    reset_password: this endpoint returns as soon as the account is
    committed, without waiting for the send, and the outcome (sent / not
    ...
    This endpoint never rolls the account back on a delivery problem,
    whatever the cause (confirmed SMTP failure, a timeout, an ambiguous
    mid-transfer disconnect — see send_reset_email's own docstring for why
    those are deliberately not distinguished any more finely than "not
    confirmed sent"). An earlier version tried to tell those apart and
    delete the account only on a "definitely no way in" outcome — which
    chased an increasingly narrow, increasingly fragile edge case (smtplib
    itself collapses a socket timeout waiting on the final acknowledgment
    into the same exception a clean disconnect raises) for a decision that
    a human admin is better placed to make anyway: check whether the user
    actually got the email, and trigger a fresh reset if they did not, once
    the situation is understood. `welcome_email_sent` on the response is
    exactly that signal, not a verdict.

    A 202 response means the account was created and a send is genuinely
    still in flight; only the welcome email's outcome is pending. Every
    other response (200/201) is already fully resolved by the time it is
    returned.
    """
    ...
    # True from the moment prepare_reset_email commits a welcome token
    # onward — including the admission-refused branch below, where
    # dispatch never ran but the token itself is already live in the DB.
    # UserOut.has_outstanding_welcome_token defaults to False (a schema
    # default, not derived from the ORM row — see users.py's
    # _outstanding_welcome_token_user_ids for the general-purpose query
    # version other endpoints use); left unset here, RegisterOut would
    # report a contradictory user state for either commit-then-fail
    # branch, since the returned response.access_token model still lists
    # `RegisterResult extends User` as its contract.
    has_outstanding_welcome_token = False
    if self_service:
        requesting_admin_id = admin.id
        # Captured as a plain int *before* prepare_reset_email runs — never
        # user.id read again after this point on the failure path.
        # prepare_reset_email's own lock-contention wrapper can call
        # session.rollback(), which expires (without detaching) this exact
        # `user` object; an implicit lazy-load on an expired instance in an
        # async session raises MissingGreenlet rather than running. It can
        # also return None because a concurrent admin deleted the account
        # outright, in which case there is no row left to serialize at all.
        user_id = user.id
        prepared = await prepare_reset_email(db, user, settings_map, welcome=True)
        if not prepared:
            # Existence-check-then-refresh: re-check by scalar id (never
            # user.id) before ever touching `user` again.
            ...
            # The row is confirmed present, but `user` itself may still be
            # the same expired instance the check above worked around (e.g.
            # lock contention elsewhere in preparation rolled back this
            # session without the account ever being deleted). Refreshing
            # now — safe, since existence is already confirmed — avoids the
            # identical MissingGreenlet the existence check itself worked
            # around, since RegisterOut.model_validate(user) below reads
            # every field on it.
            await db.refresh(user)
            welcome_email_sent = False
            welcome_link_still_valid = False
            return RegisterOut.model_validate(user).model_copy(update={
        ...
        # A live, committed token exists from this point on — see
        # prepare_reset_email's own docstring ("The token row is committed
        # before returning") — independent of whether dispatch below is
        # ever admitted.
        has_outstanding_welcome_token = True
        op_id = str(uuid4())
        ...
        # verbatim between this handler and users.py's reset_password).
        admitted = dispatch_admin_action(
            msg, smtp_cfg, user.email, user.id, token_hash,
            admin_id=requesting_admin_id, action="welcome_link", op_id=op_id,
            notify=notify_user,
        )
        if not admitted:
            # welcome_email_sent is False for the same reason in every
            # refusal case dispatch_admin_action can produce: no task
            # exists, so nothing is pending, and no further SSE outcome
            # will arrive beyond the attempted=False event it already
            # emitted. welcome_link_still_valid is True, not False: refused
            # admission means the email was never sent, but prepare_reset_
            # email already committed the token above ("A live, committed
            # token exists from this point on... independent of whether
            # dispatch below is ever admitted") and nothing here retires
            # it — it remains the account's live welcome token, exactly
            # like has_outstanding_welcome_token=True two lines below
            # already says. False here previously contradicted that same
            # field in the same response.
            return RegisterOut.model_validate(user).model_copy(update={
                "welcome_email_sent": False,
                "welcome_link_still_valid": True,
                "op_id": op_id,
                "has_outstanding_welcome_token": has_outstanding_welcome_token,
            })
    ...
    **Delivery is detached, never awaited here.** An identical body does not
    ...
    if self_service_reset_enabled(settings_map):
        ...
        if user and user.is_active:
            ...
            if prepared:
                msg, smtp_cfg, token_hash = prepared
                dispatch_reset_email(msg, smtp_cfg, user.email, user.id, token_hash)
