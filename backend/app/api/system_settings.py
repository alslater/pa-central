...
async def patch_settings(body: SystemSettingPatch, db: DbDep, user: AdminDep) -> list[SystemSettingOut]:
    ...
    for key, raw_value in body.updates.items():
        ...
        # Same class of gap as smtp_port immediately above, for the same
        # reason: smtp_host was previously validated (looks_like_smtp_host,
        # line ~333) only inside the reset_enabled block, so with
        # self-service reset off a PATCH could save an unusable value (e.g.
        # containing whitespace or control characters) with a 200 — the
        # per-key loop had no entry for this key at all. build_smtp_config
        # then treats that stored value identically to smtp_host being
        # genuinely empty ("SMTP not configured") for every consumer, not
        # only self-service reset: api/ingest.py's scan-result-notification
        # path (build_smtp_config(settings_map), ~line 224) silently
        # returns without sending or logging anything, indistinguishable
        # from SMTP never having been set up. Checked unconditionally here,
        # not through _effective() (which only matters for the
        # reset_enabled block's own "does this PATCH leave a promised
        # feature working" question) — an absent/empty value is left alone,
        # since that is the deliberate, valid "not configured" state; only
        # a value present and unusable is an error.
        if key == "smtp_host" and raw_value and not looks_like_smtp_host(raw_value):
            raise HTTPException(
                400,
                f"Setting 'smtp_host' is not a usable hostname or address "
                f"— it must not contain whitespace or control characters, "
                f"got: {raw_value!r}",
            )
