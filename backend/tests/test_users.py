...
class TestResetPassword:
    ...
    async def test_openapi_documents_the_202_pending_response(self):
        ...
        # when a send is genuinely backgrounded — FastAPI only reflects a
        # status code in the generated schema when it's declared via the
        # route's own `responses=` metadata, so a generated client would
        # otherwise never know to expect anything but 200 here.
        responses = app.openapi()["paths"]["/api/users/{user_id}/reset-password"]["post"]["responses"]
        assert "202" in responses
        schema_ref = responses["202"]["content"]["application/json"]["schema"]["$ref"]
        assert schema_ref == responses["200"]["content"]["application/json"]["schema"]["$ref"]
