...
@pytest.mark.asyncio
class TestSystemSettingsHostValidation:
    """Same class of gap as smtp_port (TestSystemSettingsPortValidation
    above), for smtp_host: looks_like_smtp_host() was previously enforced
    only inside the reset_enabled block, so with self_service_password_reset
    off a PATCH could save a syntactically unusable smtp_host (whitespace or
    a control character) with a 200.

    That mattered beyond self-service reset: smtp_host is also read by the
    independent scan-result-notification path (api/ingest.py's
    build_smtp_config(settings_map)). With reset off, an unusable host
    previously saved successfully, and build_smtp_config then silently
    returned None for every subsequent scan-result email — indistinguishable
    from SMTP never having been configured at all, with no error surfaced
    anywhere. Reproduced directly: self_service_password_reset off, PATCH
    {"smtp_host": "not a host"} returned 200, and
    build_smtp_config({"smtp_host": "not a host", ...}) returned None.
    """

    async def test_rejects_a_host_containing_whitespace_regardless_of_reset_state(
        self, client, admin_token
    ):
        r = await client.patch(
            "/api/system-settings",
            json={"updates": {"smtp_host": "not a host"}},
            headers=auth(admin_token),
        )
        assert r.status_code == 400
        assert "smtp_host" in r.json()["detail"]

    async def test_accepts_a_valid_host(self, client, admin_token):
        r = await client.patch(
            "/api/system-settings",
        ...
        assert r.status_code == 200
        settings = {s["key"]: s for s in r.json()}
        assert settings["smtp_host"]["value"] == "smtp.example.com"

    async def test_accepts_clearing_the_value(self, client, admin_token):
        """An absent/cleared value means "SMTP not configured" — that is a
        deliberate, valid state, not an error."""
        r = await client.patch(
            "/api/system-settings",
            json={"updates": {"smtp_host": ""}},
            headers=auth(admin_token),
        )
        assert r.status_code == 200

    async def test_unrelated_updates_are_unaffected(self, client, admin_token):
        """The validation must trigger only when smtp_host is actually
        present in the request body — not on every PATCH."""
        r = await client.patch(
            "/api/system-settings",
            json={"updates": {"pa_version": "1.2.3"}},
    ...
    async def test_a_bad_host_with_reset_off_and_an_unrelated_change_is_rejected(
        self, client, admin_token
    ):
        """The exact scenario the finding describes: self-service reset is
        off, and this PATCH doesn't even mention it — only the new per-key
        check (not the reset_enabled-gated effective-value check, which
        never runs when reset is off) can catch this."""
        r = await client.patch(
            "/api/system-settings",
            json={"updates": {
